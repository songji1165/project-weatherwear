...
- 슬라이드 부분을 vue 이벤트 핸들링으로 변경해 주고, DOM 접근은 queruSelect 대시 \$refs를 사용해주었다.
  > document 로 접근하여 dom의 직접적인 접근은 가상DOM기반인 vue.js와 맞지 않기 때문에 최대한 document으로 찾는 것은 피해야 한다. document접근시 mounted 후 destroyed로 인스턴스를 관리하는 방법도 있지만, 이를 좀더 깔끔하게 사용하기위해 vue에서 제공하는 것이 ref이다. DOM의 접근은 최대한 삼가하는 것이 좋지만, 우선 슬라이드에서 필요한 부분이기 때문에, ref를 사용하여 그나마 깔끔하게 코딩을 하도록 하였다.
